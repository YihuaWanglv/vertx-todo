...
/**
 * Vert.x Blueprint Application - Todo Backend Todo Service Redis Implementation
 *
 * @author Eric Zhao
 */
public class RedisTodoService implements TodoService {
	...
	public Future<Boolean> initData() {
		return insert(new Todo(Math.abs(new java.util.Random().nextInt()), "Something to do...", false, 1, "todo/ex"));
	}
	...
	public Future<Optional<Todo>> getCertain(String todoID) {
		...
		redis.hget(Constants.REDIS_TODO_KEY, todoID, res -> {
			if (res.succeeded()) {
				result.complete(Optional.ofNullable(res.result() == null ? null : new Todo(res.result())));
			} else
				result.fail(res.cause());
		});
		return result;
